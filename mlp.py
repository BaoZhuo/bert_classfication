...
## load data
train_data = pd.read_csv(os.path.join(args.raw_data_path, 'train_predic.csv'))
train_data['情感倾向'] = train_data['情感倾向'] + 1
...
train = data[data['情感倾向'] != -2]
test = data[data['情感倾向'] == -2]
...
## get train feature
del_feature = ["微博id","微博发布时间","发布人账号","微博中文内容","微博图片","微博视频","情感倾向","target_0","target_1","target_2","prediction", "albert"]
features = [i for i in train.columns if i not in del_feature]


## Convert the label to two categories
train_x = train[features]
train_y = train['情感倾向'].astype(int).values
test = test[features]
...
test_data = pd.read_csv(os.path.join(args.raw_data_path, 'test_predic.csv'))
test_data['y'] = result - 1
test_data[['id','y']].to_csv(os.path.join(args.raw_data_path, 'test_final.csv'),index=False)
